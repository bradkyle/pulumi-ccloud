...
First, clone this repo with the name of the desired provider in place of `ccloud`:

```
git clone https://github.com/pulumi/pulumi-tf-provider-boilerplate pulumi-ccloud
```

Second, replace references to `ccloud` with the name of your provider:
...
To use from Go, use `go get` to grab the latest version of the library
    ...
    $ go get github.com/pulumi/pulumi-ccloud/sdk/go/...

## Configuration

The following configuration points are available for the `ccloud` provider:

- `ccloud:apiKey` (environment: `XYZ_API_KEY`) - the API key for `ccloud`
- `ccloud:region` (environment: `XYZ_REGION`) - the region in which to deploy resources
